...
class DGGenerator:
    ...
    def generate_random_dag(self, density):
        ...
        topological_ordering = np.random.permutation(self.num_nodes)
    ...
    def generate_random_dg(self, density):
        ...
        for i in range(self.num_nodes - 1):
            target_node = np.random.randint(i + 1, self.num_nodes)
        ...
        for _ in range(num_edges):
            source_node = np.random.randint(0, self.num_nodes - 1)
            target_node = np.random.randint(source_node + 1, self.num_nodes)
