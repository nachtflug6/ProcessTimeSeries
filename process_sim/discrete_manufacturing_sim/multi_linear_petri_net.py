...
class MultiLinearPetriNet:
    def __init__(self, connectivity_graph: WeightedDirectedGraph, length_pns=2):
        ...
        self.active_transitions = torch.zeros((self.num_pns, self.length_pns), dtype=torch.bool)
    ...
    def set_initial_marking(self, pn_idx, marking):
        ...
        if len(marking) != self.length_pns:
            raise ValueError("Length of marking does not match the length of Petri net.")
    ...
    def set_capacity(self, pn_idx, place_idx, capacity):
        if pn_idx >= self.num_pns or place_idx >= self.length_pns:
            raise ValueError("Petri net index or place index is out of bounds.")
    ...
    def set_weight(self, pn_idx, weight_idx, weight):
        if pn_idx >= self.num_pns or weight_idx >= self.length_pns:
            raise ValueError("Petri net index or weight index is out of bounds.")
    ...
    def fire_transition(self, pn_idx, transition_idx):
        if pn_idx >= self.num_pns or transition_idx >= self.length_pns:
            raise ValueError("Petri net index or transition index is out of bounds.")

        # Check if the transition is enabled
        if self.active_transitions[pn_idx, transition_idx]:
            if transition_idx == 0:
                # Update markings
                self.markings[:, -1] -= self.connectivity_graph.get_previous_nodes(pn_idx)
            else:
                self.markings[pn_idx, transition_idx - 1] -= self.weights[pn_idx, transition_idx - 1]
            self.markings[pn_idx, transition_idx] += self.weights[pn_idx, transition_idx]
    ...
    def update_active_transitions(self):
        ...
        for place_idx in range(self.num_pns - 1):
            active_transitions[:, 1 + place_idx] = self.markings[:, place_idx] >= self.weights[:, place_idx]
    ...
    def print_state(self):
        markings_df = pd.DataFrame(self.markings.numpy(), columns=[f"m{i}" for i in range(self.num_pns)])
        capacities_df = pd.DataFrame(self.capacities.numpy(), columns=[f"c{i}" for i in range(self.num_pns)])
        weights_df = pd.DataFrame(self.weights.numpy(), columns=[f"w{i}" for i in range(self.num_pns)])
        active_transitions_df = pd.DataFrame(self.active_transitions.numpy(), columns=[f"t{i}" for i in range(self.num_pns)])
