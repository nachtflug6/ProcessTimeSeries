...
class ProdGraph:
    def __init__(self, adjacency, distributions, buffer_limits, batch_size=100):
        ...
        self.features = th.zeros(n_nodes, num_features)
        ...
        self.output_data = th.zeros(batch_size, num_output)
        self.log = th.empty(0, num_output)
    ...
    def get_entry(self):
        ...
        #     (features[:, 1] > 0), features[:, 1], float("inf")
        # )
        ...
        remaining_time = th.where((features[:, 1:4] > 0), features[:, 1:4], float("inf"))
